...
def main():
    ...
    half_win = win_size // 2          # 中心幀在窗口中的位置
    buf: deque = deque(maxlen=win_size)

    # 單一 smoother instance，main loop 與 tail flush 共用，保持 last_q 連續
    smoother = PoseSmoother()

    out_path = Path(args.out)
    print(f"[INFO] Output file  : {out_path.resolve()}")
    print(f"[INFO] Smooth window: {win_size} frames (Gaussian)")

    # raw_frames 存所有原始幀，用於最後 flush 尾巴
    raw_frames: list = []
    rows: list       = []
    i = 0
    ...
    while True:
        ...
        if i % args.stride == 0:
            ...
            if len(common) >= 1:
                # 每 100 幀才印一次，避免 IO bottleneck 拖慢處理速度
                if i % 100 == 0:
                ...
                if R1 is not None and R2 is not None:
                    ...
                    buf.append((timestamp, t_raw, R_raw))
                    raw_frames.append((timestamp, t_raw, R_raw))

                    # 窗口滿 win_size 時，輸出中心幀的平滑結果
                    if len(buf) == win_size:
                        sub       = list(buf)
                        center_ts = sub[half_win][0]
                        t_sm, q_sm = smoother.smooth(sub)

                        rows.append([center_ts,
                                     t_sm[0], t_sm[1], t_sm[2],
                                     q_sm[0], q_sm[1], q_sm[2], q_sm[3]])

                        print(f"frame {i:05d}  t={center_ts:.3f}s  "
    ...
    # ── flush 尾巴：輸出還沒被當作 center 的最後 half_win 幀 ──────────────────
    # 已輸出的中心幀對應 raw_frames[half_win .. n_raw - half_win - 1]
    # 尾巴中心幀索引：n_raw - half_win ~ n_raw - 1
    n_raw = len(raw_frames)
    for center_idx in range(n_raw - half_win, n_raw):
        # 取以 center_idx 為中心、盡量對稱的窗口（邊界縮減）
        lo  = max(0, center_idx - half_win)
        hi  = min(n_raw, center_idx + half_win + 1)
        sub = raw_frames[lo:hi]

        center_ts      = raw_frames[center_idx][0]
        t_sm, q_sm     = smoother.smooth(sub)   # 與 main loop 共用 smoother，last_q 連續

        rows.append([center_ts,
                     t_sm[0], t_sm[1], t_sm[2],
                     q_sm[0], q_sm[1], q_sm[2], q_sm[3]])

        print(f"[tail]        t={center_ts:.3f}s  "
              f"t=[{t_sm[0]:.3f},{t_sm[1]:.3f},{t_sm[2]:.3f}]  "
              f"q=[{q_sm[0]:.3f},{q_sm[1]:.3f},{q_sm[2]:.3f},{q_sm[3]:.3f}]")
